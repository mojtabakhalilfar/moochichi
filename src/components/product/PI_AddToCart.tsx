...
export const PI_AddToCart = ({ id }: { id: number }) => {
    ...
    return (
        <div className='flex flex-col sm:flex-row-reverse items-center justify-around space-y-2 sm:mb-12'>
            ...
            {
                getProduct(id) === 0 ?
                    <button onClick={() => handleAddToCart(id)} className='w-[342px] sm:w-[380px] sm:h-14 h-[41px] p-3 rounded-[41px] bg-[#ec6880] font-semibold text-[13px] text-white'>
                        افزودن به سبد خرید
                        |
                        {"۷,۵۰۰,۰۰۰ تومان"}
                    </button>
                    :
                    <button onClick={() => handleRemoveFromCart(id)} className='w-[342px] sm:w-[380px] sm:h-14 h-[41px] p-3 rounded-[41px] bg-[#ec6880] font-semibold text-[13px] text-white'>
                        حذف از سبد خرید
                        |
                        {"۷,۵۰۰,۰۰۰ تومان"}
                    </button>
